...
// Build SQL query
$sql = "SELECT * FROM shs_students WHERE 1=1";
$params = [];
$types = '';
...
// Set headers based on student_record.php fields for SHS
$headers = [
    'ID',
    'Student Number',
    'Last Name',
...
// Set column width for SHS Program
...
foreach ($data as $row) {
    ...
    $sheet->setCellValue('F' . $rowNumber, $row['grade_level']);
    $sheet->setCellValue('G' . $rowNumber, $row['shs_program']);
...
header('Content-Disposition: attachment;filename="shs_student_records.xlsx"');
